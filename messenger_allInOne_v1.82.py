# ============================================================
# messenger_allInOne_v1.82  변경 이력
# ============================================================
#
...
class TelegramAccountsTab(tk.Frame):
    ...
    def _show_dialogs(self):
        ...
        # ── 4. 스트리밍 callback (백그라운드 → 메인 스레드) ─────
        def _on_batch(batch: list[dict], done: bool, total: int):
            """get_dialogs_iter 콜백 — after(0) 로 UI 스레드에서 실행"""
            def _ui_update():
                ...
                if batch:
                    ...
                    _link_count_var.set(
                        f"🔗 t.me 링크 "
                        f"{len([d for d in _all_dialogs if d.get('username')])}개 추출 가능")
                if done:
                    _fetch_done[0] = True
                    n = len(_all_dialogs)
                    _hdr_var.set(f"📋 {acct_name} — 가입 채팅방 {n}개")
                    _prog_var.set(f"✅ 조회 완료 — 총 {n}개")
                    prog_bar.configure(bg="#D1FAE5")  # 완료 시 초록 배경
                    self.app._set_status(
                        f"✅ 대화방 목록 조회 완료 ({n}개) — {acct_name}")
                    chk_btn_all.config(state=tk.NORMAL)
                else:
                    _prog_var.set(
                        f"⏳ 수집 중… {total}개 (100개씩 페이지 로딩 중)")
            pop.after(0, _ui_update)

        # ── 5. 스트리밍 시작 ─────────────────────────────────────
        eng = _get_tg_engine(lambda m, lv="INFO": None)
        eng.get_dialogs_iter(acct, callback=_on_batch, batch_size=100)
        self.app._set_status(f"📋 {acct_name} 대화방 목록 조회 시작…")
