...
class TelegramAccountsTab(tk.Frame):
    ...
    def _show_dialogs(self):
        ...
        def _done(dialogs):
            ...
            _link_count_var = tk.StringVar(
                value=f"🔗 t.me 링크 {len([d for d in dialogs if d.get('username')])}개 추출 가능")
            ...
            def _copy_links_selected():
                ...
                sel = tv.selection()
                if not sel:
                    messagebox.showinfo("선택 없음", "복사할 채팅방을 선택하세요.\n(Ctrl+클릭으로 다중 선택 가능)")
                    return
                links = []
                for item in sel:
                    vals = tv.item(item, "values")
                    ...
                    if uname and uname != "-":
                        ...
                        clean = uname.lstrip("@")
                        links.append(f"https://t.me/{clean}")
                if links:
                    pop.clipboard_clear()
                    pop.clipboard_append("\n".join(links))
                    _link_count_var.set(
                        f"✅ {len(links)}개 복사됨 — Ctrl+V로 붙여넣기")
                else:
                    messagebox.showinfo("링크 없음",
                        "선택된 채팅방에 @링크가 없습니다.\n(private 그룹은 링크 없음)")

            def _copy_links_all():
                """전체 목록의 t.me 링크 복사 (username 있는 것만)"""
                links = []
                for d in dialogs:
                    uname = d.get("username", "")
                    if uname:
                        links.append(f"https://t.me/{uname}")
                if links:
                    pop.clipboard_clear()
                    pop.clipboard_append("\n".join(links))
                    _link_count_var.set(
